...
import { MessageSquare, X, Send, User, Minus, Square } from 'lucide-react';
...
export default function ChatModal({ 
  ...
  return (
    <motion.div
      ...
      className={`fixed bottom-4 right-4 z-[1000] w-full max-w-[360px] ${isMinimized ? 'h-[72px]' : 'h-[500px]'} bg-zinc-950 border border-white/10 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden transition-all duration-300 ease-in-out`}
      style={{ borderColor: `${themeColor}33` }}
    >
      <div 
        className="p-4 bg-zinc-900/50 border-b border-white/5 flex justify-between items-center backdrop-blur-md cursor-pointer"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-zinc-500 font-mono mt-1 uppercase tracking-wider">
              {isMinimized ? 'Tap to expand' : 'Secure Channel'}
            </div>
          </div>
        </div>
